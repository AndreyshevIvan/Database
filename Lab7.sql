...
3 Дать среднюю оценку студентов по каждому предмету для тех предметов,
  ...
  SELECT valuation.id_valuation, AVG(valuation.mark) FROM valuation
  WHERE
  valuation.id_lesson IN (
    SELECT lesson.id_lesson FROM
      ...
      GROUP BY subject.id_subject, lesson.id_subject HAVING COUNT(DISTINCT student.id_student) > 3
  )
